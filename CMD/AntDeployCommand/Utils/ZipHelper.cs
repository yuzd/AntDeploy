...
namespace AntDeployCommand.Utils
{
    public class ZipHelper
    {
        ...
        public static MemoryStream DoCreateTarFromDirectory(string sourceDirectory, List<string> ignoreList = null, Func<int, bool> progress = null, Action<string> logger = null)
        {
            ...
            foreach (FileSystemInfo enumerateFileSystemInfo in allFile)
            {
                ...
                if (enumerateFileSystemInfo is FileInfo)
                {
                    ...
                    {
                        logger?.Invoke($"Find Dockerfile In Package: {entryName}");
                    }
                    haveFile = true;
        ...
        public static MemoryStream DoCreateTarFromDirectory(string sourceDirectory, List<string> fileList, List<string> ignoreList = null, Func<int, bool> progress = null, Action<string> logger = null, bool isSelectDeploy = false)
        {
            MemoryStream outputMemStream = new MemoryStream();
            TarArchive tarArchive = TarArchive.CreateOutputTarArchive(outputMemStream);
            tarArchive.RootPath = sourceDirectory.Replace('\\', '/');
